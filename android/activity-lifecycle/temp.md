Активность запускается, выполняется ее метод onCreate(). Выполняется код инициализации активности из метода onCreate(). В этой точке активность еще не видна, так как метод onStart() еще не вызывался.

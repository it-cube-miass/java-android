...
```
Для приведения возвращаемого значения к правильному типу (в нашем случае TextView), используется операция приведения типа.
...
Видео, если возникли трудности

{% embed url="https://youtu.be/xndAsrCYNSc" %}
...
{% embed url="https://youtu.be/0Socn0sfogo" %}
...
Нам необходимо увеличивать значение seconds на 1 каждую секунду, когда секундомер запущен. Поэтому нам потребуется какой-нибудь таймер.
